...
describe("Verifier", () => {
  ...
  describe("runAutomatedChecks", () => {
    ...
    it("should use init script mode when useInitScript is true", async () => {
      // Create a mock init script
      const aiDir = path.join(testDir, "ai");
      await fs.mkdir(aiDir, { recursive: true });
      const scriptPath = path.join(aiDir, "init.sh");
      ...
      expect(results).toHaveLength(1);
      expect(results[0].type).toBe("init-script");
    });

    it("should add --quick flag in init script mode when testMode is quick", async () => {
      const aiDir = path.join(testDir, "ai");
      await fs.mkdir(aiDir, { recursive: true });
      const scriptPath = path.join(aiDir, "init.sh");
      // Script that outputs the arguments it receives
      await fs.writeFile(scriptPath, '#!/bin/bash\necho "args: $@"\nexit 0');
      await fs.chmod(scriptPath, 0o755);
    ...
    it("should add --skip-e2e flag in init script mode when skipE2E is true", async () => {
      const aiDir = path.join(testDir, "ai");
      await fs.mkdir(aiDir, { recursive: true });
      const scriptPath = path.join(aiDir, "init.sh");
      await fs.writeFile(scriptPath, '#!/bin/bash\necho "args: $@"\nexit 0');
    ...
    it("should pass E2E_TAGS env var in init script mode", async () => {
      const aiDir = path.join(testDir, "ai");
      await fs.mkdir(aiDir, { recursive: true });
      const scriptPath = path.join(aiDir, "init.sh");
      await fs.writeFile(scriptPath, '#!/bin/bash\necho "success"\nexit 0');
      ...
      // Test that e2eTags option is accepted and results are returned
      // Note: The actual env var passing is tested through the command execution
      expect(results).toHaveLength(1);
      expect(results[0].type).toBe("init-script");
    });

    it("should handle init script failure", async () => {
      // Use the mock to simulate failure
      setExecMockWithErrors((cmd: string) => {
        if (cmd.includes("init.sh")) {
          const error = new Error("Script failed") as Error & {
            stdout: string;
            stderr: string;
          };
          error.stdout = "Test failed";
          error.stderr = "";
          return error;
        }
        return { stdout: "" };
    ...
    it("should include test pattern in init script command for quick mode", async () => {
      const aiDir = path.join(testDir, "ai");
      await fs.mkdir(aiDir, { recursive: true });
      const scriptPath = path.join(aiDir, "init.sh");
      await fs.writeFile(scriptPath, '#!/bin/bash\necho "args: $@"\nexit 0');
